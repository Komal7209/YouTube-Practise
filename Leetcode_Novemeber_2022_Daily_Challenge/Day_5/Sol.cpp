...
public:
   // bool is_end;
    ...
    {
       // is_end = false;
        children = vector<TrieNode *>(26, NULL); // 26 memory blocks reserved
    }
};
...
public:
    ...
    {
        TrieNode *root = new TrieNode();// allocating memory in runtime of type TrieNode in bytes
    ...
    { 
        char c = board[i][j];
        if (c == '#' || !p->children[c - 'a']) // c when acting as index compares if that word is present in trie or not
            return;
        p = p->children[c - 'a'];
        if (p->word.size() > 0)
        {
